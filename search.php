<?php
    ...
    if(isset($_GET['search']))
    {
        ...
        $sql_q = "SELECT * FROM users WHERE first_name LIKE '%$search%' || last_name LIKE '%$search%' limit 30";
        $DB = new Database();
        $results = $DB->read_from_db($sql_q);
                             ...
                             if(is_array($results))
                                {
                                    foreach($results as $row)
                                    {
                                        ...
                                        $FRIENDS_row = $account_user->identify_user($row['userid']);
